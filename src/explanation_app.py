...
client = Anthropic(
    api_key=os.getenv(
        "ANTHROPIC_API_KEY"
    )  # this is my colab setup: update to relevant key location
)
...
def get_summary(url, max_section=7):
    ...
    section_ids = [
        section for section in section_ids[0:max_section] if section is not None
    ]
...
def get_line_by_line_summary(url, max_section=5):
    ...
    for i in range(max_section):
        ...
        nested_output[i] = {"original": original_text, "explanation": out_text}
